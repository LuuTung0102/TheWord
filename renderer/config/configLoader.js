      ...
      if (!foundMatch) {
        ...
        if (schemaField) {
          ...
          if (placeholder.includes("Address")) {
            ...
            // Check if this is an auto-generated placeholder (should not be in config)
            const isAutoGenerated = /^NameT\d+$/.test(placeholder);
            
            if (isAutoGenerated) {
              // Auto-generated placeholders don't need config mapping
              // They will be created automatically in generate.js
              mapping[placeholder] = {
                type: 'text',
                ...
                subgroup: 'INFO',
                required: false,
                hidden: true // Hidden vì không cần hiển thị trong form
              };
              console.log(`✅ Auto-generated placeholder ${placeholder} - no config needed`);
            } else {
              // Fallback: create default mapping for other placeholders
                  let autoLabel = placeholder;
                  autoLabel = autoLabel.replace(/[0-9_]+$/, '');
                  autoLabel = autoLabel.replace(/([A-Z])/g, ' $1').trim();
                  autoLabel = autoLabel.charAt(0).toUpperCase() + autoLabel.slice(1);
